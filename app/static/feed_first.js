  ...
  root.addEventListener("click", function (event) {
    ...
    if (statement) {
      ...
      postJSON("/api/feed-first/statement", {
        ...
        const itemId = (root.querySelector("[data-reader-root]") || {}).dataset
          ? root.querySelector("[data-reader-root]").dataset.itemId
          : "";
        if (itemId) {
          const cards = Array.prototype.slice.call(root.querySelectorAll("[data-statement-id]"));
          const remaining = cards.map(function (card) {
            if (card.dataset.statementId === data.statement.id) return data.statement;
            return {
              id: card.dataset.statementId,
              statement_state: card.dataset.statementState,
              statement_text: (card.querySelector("textarea") || {}).value || "",
              supporting_passages: [],
              support_locators: [],
            };
          });
          renderStatements(remaining);
        }
      });
      return;
    }
