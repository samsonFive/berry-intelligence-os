...
The AppShell (Work / Decide / Monitor / Library / System, berry context selector, offcanvas mobile nav, Feed grid/compact, slide-over Reader, collapsed desktop icon rail) is a bounded prototype. **Company Profile is in this prototype.** Do not migrate Landscape, Sources, Watches, Assessment, queues, or admin/system pages unless explicitly asked. Bootstrap 5.3 is vendored at `app/static/vendor/bootstrap/` for offcanvas/collapse/grid conventions only. Do not import Mirbal, Mooli, or chart/calendar vendor bundles. Nav action counts use `.v2-count-action` (not purple inventory pills). Berry context is a selector, not a per-crop theme; Landscape in Library follows the selected berry (`/entities/berry` when Global). Desktop hamburger collapses the persistent sidebar into an accessible icon rail (`title` + `aria-label` + action counts); below 1100px it opens `#v2NavOffcanvas`. Feed overlay is `#v2ReaderOffcanvas` and is shared with Company Recent Intelligence; `j`/`k` while it is open load adjacent items into the same pane. Escape closes it and returns focus to the triggering card. Compact is throughput mode (SOURCE · TIME / headline / metadata / marks; actions on hover, keyboard-current, or focus). Overlay `/api/intelligence/{id}/reader` must not run Morning Brief nav work. Full V2 migration of remaining pages is not approved yet.
