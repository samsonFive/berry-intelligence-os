...
The AppShell (Work / Decide / Monitor / Library / System, berry context selector, offcanvas mobile nav, Feed grid/compact, slide-over Reader) is a bounded prototype. Do not migrate Company, Landscape, Sources, Watches, or Assessment pages unless explicitly asked. Bootstrap 5.3 is vendored at `app/static/vendor/bootstrap/` for offcanvas/collapse/grid conventions only. Do not import Mirbal, Mooli, or chart/calendar vendor bundles. Nav action counts use `.v2-count-action` (not purple inventory pills). Berry context is a selector, not a per-crop theme; Landscape in Library follows the selected berry (`/entities/berry` when Global). Desktop hamburger collapses the persistent sidebar; below 1100px it opens `#v2NavOffcanvas`. Feed overlay is `#v2ReaderOffcanvas`; `j`/`k` while it is open load adjacent items into the same pane. Escape closes it and returns focus to the triggering card.
