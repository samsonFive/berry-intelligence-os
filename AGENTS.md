...
Claim Testing (`/queues/testing`) follows the berry context selector. The live tagged corpus is blueberry-only, so a leftover `inbox/analyst_queue_state.json` `meta.ui.berry` of strawberry/raspberry/blackberry shows an honest empty queue. Set context to Global or Blueberry before judging volume. Testing pages must not call Morning Brief, Story Thread grouping, Global Search indexing, collector scans, or `variety_footprint`.
...
Migrated V2 surfaces: Morning Brief, Live Intelligence Grid/Compact, ReaderOffcanvas, Signal Review, Company Profile, Pending Review (`/pending`), Reading Queue, Assessments, **Monitor** — Watches + Alerts (`/queues/monitoring`), Source Health (`/sources`), **Variety Intelligence** (`/entities/variety`, `/entities/variety/{id}`, `?view=compete`, `?view=observations`), **Global Intelligence Search** (`#v2SearchOffcanvas`, `/search`, `/api/search/global`), **Claim Testing / Testing Queue** (`/queues/testing`, `/queues/testing/{id}`).

Do **not** migrate Landscape, Sources inventory-config admin, or remaining admin/system catalogs except global regression fixes. Landscape waits until Trade / Retail / Registry workstreams in `docs/v2/INTELLIGENCE-EXPANSION-BUILD-GUIDE.md` settle (Workstreams D, E, G, H). Stop after Claim Testing / Testing Queue V2: do not begin Landscape, Learner Mode, AI claim adjudication, AI/RAG/vector search, a Trade/Weather UI, or remaining admin in the same change. You may link Landscape ↔ Varieties; do not redesign Landscape.
...
**Claim Testing is verification, not Learner Mode and not Fact publication.** `/queues/testing` is the existing priority-tagged Evidence workflow (`priority.testing.level != none`) plus the overlay states `needs_testing` / `pass` / `fail` / `defer`. A Testing item is not a first-class Claim object. A Fact with `classification: claim` is a different record. Atomic review (`/review?kind=atomic`, System → Claims) is a different gate. Pass / Fail / Defer never write `data/facts/` and never imply `tested = Fact`. Supporting and contradicting Evidence come only from existing `evidence_links` (`corroborates` / `contradicts`); repeated claims are not independent corroboration. Reuse `#v2ReaderOffcanvas` and canonical Company / Variety / Geography / Berry routes — do not invent a Testing article modal or duplicate entity profiles. Testing pages must not call Morning Brief, full Story Thread grouping, Global Search indexing, collector scans, or `variety_footprint`. Learner Mode may later earn an “Explain this concept” link; do not create Learner topics, glossaries, or agronomy pages from this surface.
...
| Claim testing | tagged evidence still `needs_testing` | Pass / Fail / Defer (Reopen from completed). This is claim verification, not `scripts/qualify_extraction_model.py`, not Learner Mode, and not Fact publication. |
