...
def test_fresh_cache_does_not_invoke_translation_or_image_network(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.feed_first_translate.perplexity_translator",
        unexpected_network,
    )
    monkeypatch.setattr(
        "app.services.feed_first_reader.fetch_source_preview_image",
        unexpected_network,
    )
    monkeypatch.setattr(
        "app.services.feed_first_reader.fetch_publisher_home_preview",
        unexpected_network,
    )
