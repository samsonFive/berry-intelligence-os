...
def test_windows_launcher_waits_for_server_and_opens_external_browser():
    ...
    assert "BIOS_START_PATH" in source
    assert '$appUrl = "http://${browserHost}:${port}${startPath}"' in source
...
def test_human_gate_launcher_opens_unreviewed_statements():
    source = Path("scripts/run_human_gate.ps1").read_text(encoding="utf-8")
    assert 'BIOS_START_PATH = "/statements?review=unreviewed"' in source
    assert "run_local.ps1" in source
    docs = Path("docs/LOCAL-FEED-FIRST.md").read_text(encoding="utf-8")
    assert r".\scripts\run_human_gate.ps1" in docs
    assert "Approve" in docs
    assert "Remove" in docs
