...
The final pre-deploy backup is `/var/backups/berry-intelligence-os/berry-runtime-20260823T205936Z.tar.gz`, independently verified with SHA-256 `133ec457488cbae3ddfa6c413faafa0d7c5750818c78bbe0264d5489b65a1cdc`; its manifest covers `data` and `inbox`. Post-deploy comparison against that manifest found all 2,656 data files and all 1,556 inbox Evidence files intact. The only path differences were removal of the ephemeral backup-held collection lock, two expected timer log files, and regeneration of the body-free recovery manifest.

Live proof: `/healthz` 200 and Docker healthy; `bios-collection.timer` enabled/active; `/login` 200; unauthenticated `/source-fidelity` 302; login 303; authenticated `/source-fidelity` 200 with the queue marker present. `BIOS_COLLECTION_ENABLE_EXTRACTION` is unset, qualification-marker count is zero, staged source-fidelity artifact count is zero, and source-fidelity review-event count is zero. The private production manifest is `/app/runtime/inbox/operations/source-fidelity-recovery/recovery-manifest.json`; it contains hashes/metadata only and no body keys.
