...
**Status:** Planning draft, not accepted. See `08-DECISION-LOG.md` for every decision implied here, all marked `PROPOSED`.
...
| NOW | V2 | LATER |
|---|---|---|
| No database. Flat JSON files, one per record, read fresh (with an in-process mtime-based cache) on every request. | **New, operational store — with JSON remaining the interchange contract (Decision D-001, D-002).** PostgreSQL becomes where the app reads/writes at runtime; every table maps closely to a domain-model object; every row remains exportable back to the same JSON shape the Intelligence Package spec defines, so "the data" is never something only readable through a live database connection. See `07-IMPLEMENTATION-ROADMAP.md` Phase 3 for the parity-migration approach (dual-write, not big-bang cutover). | Managed multi-region replication, read replicas, sharding — none justified at current or near-term data volumes (V1's entire dataset is under 2,000 records across all types). |
