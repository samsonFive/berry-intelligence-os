...
Before any mission deployment mutation, the durable bind-mounted backup `/app/backups/berry-runtime-20260824T145738Z.tar.gz` was created and independently verified with SHA-256 `86896058987bd407be71718bec6c548b33d3b87fc27722e5baba0ac5e88c880e`; its manifest contained 12,677 runtime files. Startup synchronized the mounted cadence policy through the authoritative contract: repository seed and runtime both hashed to `bcf40476e42156a81fc6215a92b1c886324c95ef96bb4b08a3b5226d3c5675b8`.
...
The no-write dispatcher proof evaluated 58 article/news candidates, selected exactly four due, skipped 54 not due, and paused the existing blocked Growing Produce Source through Source Health. It evaluated 12 spoken-media Sources and selected none because all were not due. Both runs reported extraction disabled, unconfigured, unqualified, and unrunnable. Restoring the existing timer triggered the same registry dispatcher rather than an all-Source sweep. Live run `collection-20260824T152848982388Z` completed successfully in 424 seconds: Fruitnet, berry trade remedy, global caneberry, and Driscoll's were the only four Sources checked; all succeeded, producing seven new discoveries and three private publication-review drafts with zero extraction, transcription, Atomic proposal, or review action. The timer scheduled its next normal 15-minute dispatcher check and released the collection lock.

Mounted state survived the rebuild: data remained 2,657 files, inbox 10,020 files, and Evidence 1,660 files before the scheduler smoke. A verified restore of the pre-mutation backup was compared with live state after the smoke: all 1,660 prior Evidence files and all 11 review-event files were byte-identical, and the Evidence diff contained only the three newly created private drafts. Post-smoke counts were 2,657 data, 10,043 inbox, 1,663 Evidence, and 11 review-event files. No qualification marker existed, both internal and public `/healthz` returned 200, the app was healthy, the operator status completed in 4.03 seconds, and the timer was enabled/active with no lingering lock. TD-076 remains visible rather than bypassed: production runtime exercises 70 discoverable Sources until the three protected divergent Source records receive an explicit ownership resolution, while the deterministic policy and audit cover all 73 canonical Sources.
