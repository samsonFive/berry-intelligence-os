...
See `docs/v2/TECHNICAL-DEBT-REGISTER.md` for the full entries (TD-024 through TD-029): HS-code ambiguity (Part 2), the unregistered-API-key gap for US Census, undocumented Comtrade rate limits, no revision/resubmission handling (a "final" period can later be revised by the reporting country; this mission's pilot does not re-fetch and diff already-drafted periods), no currency normalization beyond Comtrade's own USD-only convention, no country-naming reconciliation beyond the 7 geographies this pilot's own lookup table covers, and the CIF-vs-FOB value-basis distinction being carried but not yet surfaced in any derived metric (a YoY value comparison across an import lane and an export lane for the "same" flow would silently mix bases if a caller weren't careful -- flagged, not fixed, since no real caller does this yet).
