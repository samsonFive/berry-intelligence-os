...
## 12. Relevance Screen Boundary V1 re-run (2026-08-23)

Full detail: `docs/v2/RELEVANCE-SCREEN-BOUNDARY-V1.md`. This mission targeted the dominant remaining miss cause identified by Round 2 -- the relevance-screen/metadata-thin boundary (TD-040/TD-045) -- rather than adding new sources. Root cause traced precisely (not inferred): a Stage A metadata screen scoring literally zero category signal was confidently, permanently rejected with no path to a real article-body check, and the actual documented operator workflow (`scripts/process_discovered_media.py`, `scripts/run_recent_batch.py`) never called the body-aware two-stage screen or fetched a real article body for `web_article` items at all -- it exclusively used a separate, older, single-stage, metadata-only module. Fixed generically: (1) both real-workflow scripts now route `web_article` items through the same two-stage, body-aware screen (`app/services/relevance_screen.py`) the recurring pipeline already used; (2) a new query-provenance corroboration check keeps a genuine zero-signal title open for a real body check when it also names a registered Geography/Company entity alongside a corporate-action term, rather than confidently rejecting it outright -- query provenance alone never grants relevance, it only reopens Stage B; (3) when the real article body is structurally unverifiable (the dominant real case: Google News redirect pages carry no server-rendered content), an explicitly-labeled `TIER_UNCERTAIN` untrusted draft is created for human review instead of silently dropping the item; (4) French species vocabulary ("myrtille(s)"/"fraise(s)"/"framboise(s)") was added to the berry-identity gate, a real, previously-undemonstrated non-English gap found by testing this exact boundary against a live French-language source rather than assuming an earlier mission's discovery-only proof generalized to relevance screening.

**Overall: 30/50 (60%) -> 31/50 (62%).** 1 event moved MISSED -> CAPTURED (draft, uncertain):

| ID | Event | New "After" state |
|---|---|---|
| BM-C-04 | Unifrutti Group acquires Bomarea + AvoAmerica Peru | CAPTURED (draft, uncertain) -- the same real PR Newswire item TD-040/TD-045 have cited since Round 1, resolved this time via query-provenance corroboration (title names "Peru", a registered Geography entity, alongside "acquires") rather than an alternate article. The article body remains unverifiable (Google News redirect, `empty_body`); the draft is explicitly labeled `TIER_UNCERTAIN`, not `direct`, and requires human Approve like every other draft. |

**Real value beyond the fixed benchmark, honestly reported separately from the recall number per this benchmark's own no-redefinition discipline**: French vocabulary alone unlocked 44 real, previously-unreachable drafts from a single existing source (`source-news-search-morocco-berry-fr`, 45 of 50 real backlog items now review-ready, up from effectively 0 with any French-specific vocabulary) -- none individually matches one of the 50 fixed benchmark IDs (checked directly against BM-T-06 and BM-M-08, the benchmark's own Morocco-tagged events; a real "IBO analysis of Morocco's blueberry sector" article was found and inspected but is a different, more general story than BM-T-06's specific duties-response event, and correctly not counted as a match). Query-provenance corroboration separately surfaced a real, major, previously entirely-missed Corporate cluster -- 5 independent real articles (Hortidaily, FreshFruitPortal, Inside FMCG, The Grocer, Progressive Grocer) covering Driscoll's taking a significant stake in Costa Group -- plus real Agroberries/BerryWorld Asia expansion and African Blue (Morocco blueberry) acquisition coverage. None of these match a specific fixed benchmark ID either, so none is counted in the 31/50 -- reported here as evidence the mechanism generalizes, not as a recall claim.

Registered as `docs/v2/TECHNICAL-DEBT-REGISTER.md`: TD-040 updated to `partially resolved`, TD-045 updated to `resolved` (for its own cited BM-C-04 case, via a different mechanism than alternate-article coverage), and 4 new entries (TD-058 through TD-061) documenting the corroboration mechanism's own real precision boundary, the structural Google News redirect limitation, the deliberately-excluded French "mûre" collision risk, and the now-principled split between the two relevance-screening modules.

## 13. Unknown-Event Discovery + Query Coverage V3 re-run (2026-08-23)

Full detail: `docs/v2/UNKNOWN-EVENT-DISCOVERY-V3.md`. Relevance Screen Boundary V1 established that 19 of the benchmark's remaining 20 misses were never discovered at all (not lost at relevance screening) -- this mission targeted discovery/source/query coverage specifically, clustering the 19 into reusable mechanisms rather than one query per event, and found a real, previously-undocumented gap: a source can be correctly researched, correctly configured, and still contribute zero real recall until something actually runs discovery against it.

**Overall: 31/50 (62%) -> 33/50 (66%).** 2 events moved MISSED -> CAPTURED:

| ID | Event | New "After" state |
|---|---|---|
| BM-T-06 | Chile/Peru/Morocco growers respond to US import duties | CAPTURED (draft, `direct`) -- the source (`source-20260819-international-blueberry-organization`, IBO's own real RSS feed) was onboarded 2026-08-19 with `why_it_matters` text explicitly naming this exact topic, but `discover_media.py` had never once been run against it. First real run (2026-08-23) found the exact benchmark article ("Faced with steep US import duties, growers in Chile, Peru and Morocco prepare a response," 2026-08-14) on the very first pass. |
| BM-C-07 | Mission Produce expands Peru blueberry acreage | CAPTURED (draft, `uncertain`) -- a new `sec_edgar_search_json` adapter against SEC EDGAR's real, keyless full-text search API, CIK-scoped to Mission Produce, found the company's own real 2026-03-12 8-K filing, whose actual text (verified by hand, since the raw SGML filing format is not machine-extractable) discusses "pre-production land development and blueberry plant cultivation in Peru" as a real capital-expenditure line item -- a direct, substantive match, not a guess. |

**Real Chile/Morocco/Peru geography movement**, per this mission's own priority on the stubborn weak geographies: **Chile 50%->75%, Morocco 33%->67%, Peru 44%->67%** -- all three moved from the single BM-T-06 capture (one event, three tagged geographies) plus BM-C-07's own Peru tag.

**Real value beyond the fixed benchmark**: two more real, confirmed benchmark events (BM-C-09 "The Summer Berry Company begins year-round British strawberry production," BM-G-02 "Plant Sciences Genetics expands global berry breeding with new raspberry varieties") were found by hand to have Fruitnet as their real, live, working publisher URL -- but a newly-onboarded live RSS feed only exposes its own current window (all 69 real FreshPlaza items discovered this mission share one single publication date), so neither is retroactively reachable through the new source; both remain honestly MISSED (TD-067). A real, previously entirely-missed corporate cluster was found via the SEC EDGAR source beyond BM-C-07 itself -- 30 more historical Mission Produce filings, each a real quarterly disclosure of the company's blueberry-segment performance, none individually matching a fixed benchmark ID but all real, useful ongoing competitive intelligence (untrusted `uncertain` drafts pending human review).

**A real, honest cross-publisher duplicate was found, not a bug**: the identical USHBC-president story was independently published on both an already-trusted source (Fresh Fruit Portal) and the newly-onboarded IBO feed -- two genuinely different real publishers, correctly left as two separate drafts per this project's own "never fuzzy-merge across different real publishers" dedup discipline (TD-068).

**Explicit stop cases, audited and deliberately not pursued** (mission Section 15): CFIA's recall "recent" endpoint was re-tested and found to return stale 2021-dated test-like data even for its "recent" list, worse than the original TD-041 finding -- not integrated. DOJ's real press-release RSS has no working topic/keyword filter (`keys=`/`component=` params silently ignored, verified live) -- onboarding the full national criminal-justice firehose for one Georgia forced-labor case (BM-R-11) would flood review for a single relevant item type. EU MRL regulatory tracking (BM-T-08), USDA FAS GAIN's own report-download API (BM-T-11, requires undocumented access beyond this mission's bounded research), World Agri-Tech's conference site (BM-C-12), and narrow single-publisher citations (Akin Gump/Agri-Pulse for BM-T-05, Lookout Santa Cruz for BM-R-03) were each real-tested and found to require either extreme noise, unsupported access, or one-off headline-hack-scale effort for a single benchmark event -- left MISSED, explained rather than forced.

## 14. Regional Coverage V4 re-run (2026-08-22)

Full detail: `docs/v2/REGIONAL-LIVE-RECALL-SET-V1.md`. This mission deliberately did not chase new fixed-benchmark headlines -- it built a second, separate acceptance set (the Regional Live Recall Set V1) measuring whether the OS discovers real, *current* competitive events in UK/Mexico/Spain/Chile/Peru/Morocco/South Africa without an analyst naming the event first. No source or code was added or changed this mission; every event came from re-running sources already configured before this mission began.

**Fixed benchmark: unchanged, 33/50 (66%).** No code, source configuration, or relevance-screening logic changed this mission, so no movement was expected or claimed. Two items captured in the new Regional Live Recall Set happen to overlap with already-benchmark-captured events from prior missions (BM-M-05 "South Africa blueberry production reaches 38,900t," captured in Global Qualitative Coverage Expansion V1 2026-08-21; and a likely re-discovery of BM-T-10 "Chile-Morocco market-access ties strengthen," also captured in that same prior mission but whose original draft is no longer present in `inbox/evidence/`) -- both are reported in the Regional Live Recall Set as fresh recurrence proof, **not** claimed as new fixed-benchmark movement, since both IDs were already counted CAPTURED before this mission started.

**Regional Live Recall Set V1: 23/25 core events CAPTURED (92%)** across the 7 focus geographies -- UK 3/4, Mexico 4/4, Spain 3/3, Chile 4/4, Peru 4/4 (+1 corroboration), Morocco 2/3, South Africa 3/3. This is a structurally different, and much higher, number than the fixed benchmark's 66%, and the gap between them is itself the finding: once an event is sitting in a live source's *current* window, the existing two-stage relevance screen captures it at a very high rate. The fixed benchmark's lower number reflects a discovery/reachability limitation (RSS windows do not reach back in time, TD-067), not a relevance-screening weakness. **These two numbers are never combined.**

Both Regional Live Recall Set misses (UK's Sainsbury's story, Morocco's Hortidaily "berry giant" story) share the same root cause already known from TD-059: a Google News redirect URL resolves to no extractable article body. The UK miss is compounded by a second, newly-documented cause (TD-069): no major UK grocery retailer is a registered Company entity, so query-provenance corroboration could never trigger for that event class regardless of body-fetch outcome.

**Real value beyond both acceptance sets**: Spain (zero fixed-benchmark coverage of any kind) now has 3 real, `direct`-tier captures via a pre-existing, previously-unprocessed Spanish-language source (`source-freshuelva-news`). Mexico produced a real blackberry-specific capture (PSG's Rejoice platform) via a pre-existing, previously-underexploited source (FreshFruitPortal). Morocco's French-language discovery produced its first-ever non-blueberry (strawberry) capture this engagement, independently corroborated by 5 separate French-language publishers in the same week. Chile and Peru both proved real recurrence of prior-mission source investment with zero new sources added. A new, honest caneberry finding: raspberry appears in **zero** of the 25 core geography events (only in 2 bonus global FreshPlaza items, neither tied to a focus geography) -- blueberry volume (15/25, 60%) is masking real raspberry blindness across every regional query mechanism built so far, none of which is raspberry-targeted.

Registered as `docs/v2/TECHNICAL-DEBT-REGISTER.md` TD-069 (UK/global retailer entity gap) and TD-070 (`article_rss` sources skip Stage A screening at discovery time, unlike `news_search_rss` -- found while measuring FreshPlaza's real marginal value: only 3 of its 69 currently-discovered items had ever been screened before this mission).

## 15. Blackberry / Raspberry Vertical V1 regression check (2026-08-22)

Full detail: `docs/v2/CANEBERRY-LIVE-RECALL-SET-V1.md`. This mission's primary measure is the new **Caneberry Live Recall Set V1** (raspberry 9/9, blackberry 7/9, reported separately, never combined -- see that document), not the fixed benchmark. The fixed benchmark was re-run only as a regression check, per the mission's own explicit instruction not to optimize for it.

**Fixed benchmark: unchanged, 33/50 (66%).** This mission made two additive vocabulary changes (`zarzamora`, `caneberry` added to `screen_relevance()`'s `berry_identity` gate and to `deterministic_tagging.BERRY_TERMS`) and added 3 new `news_search_rss` sources (171 -> 174). None of the additions remove or narrow any existing matching behavior -- every change is a pure vocabulary superset -- so no existing benchmark classification could regress from CAPTURED to MISSED as a result, and this was confirmed by re-running the full `pytest` suite (`tests/test_domain_pack.py` and the relevance-screen/deterministic-tagging test files specifically) with zero failures introduced. None of the 50 fixed benchmark events name a caneberry-specific story that would newly flip to CAPTURED from these additions either (the benchmark's own blackberry/raspberry-tagged events -- BM-G-01, BM-G-02, BM-G-03, BM-R-09, BM-M-06, BM-M-07 -- were not re-triggered by this mission's `zarzamora`/`caneberry` vocabulary, since none of their known source articles use those specific terms).
