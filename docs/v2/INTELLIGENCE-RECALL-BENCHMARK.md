...
Registered as `docs/v2/TECHNICAL-DEBT-REGISTER.md`: TD-040 updated to `partially resolved`, TD-045 updated to `resolved` (for its own cited BM-C-04 case, via a different mechanism than alternate-article coverage), and 4 new entries (TD-055 through TD-058) documenting the corroboration mechanism's own real precision boundary, the structural Google News redirect limitation, the deliberately-excluded French "mûre" collision risk, and the now-principled split between the two relevance-screening modules.
