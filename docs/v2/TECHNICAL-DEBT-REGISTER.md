...
| **Recommended resolution** | Human-review the three deterministic recoveries only after explicit additive staging, investigate the repeated-body acquisition failure, preserve full article bodies on future publication (TD-081 is resolved), and design a separate compare-and-review reacquisition path for the remaining 1,159 records. Never silently refetch or overwrite trusted records. |
| **Status** | active |
| **Owner lane** | data / ops |
| **PR/SHA when resolved** | — |
