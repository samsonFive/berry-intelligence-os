...
| TD-093 | trusted extraction source fidelity | Combined local/production audit finds two recoverable articles and one explicit-lineage transcript; 65 repeated-body candidates are blocked as conflicts and 1,159 of 1,227 thin records have no historic artifact. | High | active (safe recovery path shipped) | `scripts/source_fidelity_recovery.py`; `docs/v2/TRUSTED-SOURCE-FIDELITY-RECOVERY-V1.md` |
...
| **Date discovered** | 2026-08-23 |
| **Evidence** | Atomic Extraction Backlog Readiness V1 found 36 ready registry records, 1,227 thin descriptions, three fictional fixtures, and two duplicates in the 1,268-record production corpus. Combined local/production recovery finds Pink Hudson's exact-ID/exact-URL five-paragraph article, one exact-URL Hortifrut article, and one Lucentlands `discovered_item_id` lineage match to a 2,229-segment normalized transcript. Production proof also found 65 distinct Google News publication URLs sharing one identical 1,356-character body; all are now blocked as repeated-body conflicts rather than overstated as independent recoveries. Zero ambiguous matches exist; 1,159 records have no artifact across the combined locations. |
| **Impact** | A qualified model could immediately process only a small, almost entirely Blueberry registry subset. After three separate human source-fidelity affirmations, two articles and one transcript could join it, raising realistic readiness from 36 to 39. Blackberry and broad source-type diversity remain absent. Running the rest of the legacy population would still extract from summaries rather than source material. |
| **Workaround** | Use `scripts/source_fidelity_recovery.py` only in dry-run or explicit-ID additive mode. Recovered bodies remain private and extraction-ineligible until a human source-fidelity affirmation. Never substitute pending drafts or current refetches for historic trusted sources. |
| **Recommended resolution** | Forward prevention and the bounded compare-and-review reacquisition path are shipped in Rich Source Acquisition + Selective Historical Reacquisition V1. Human-review the three deterministic historic recoveries only after explicit additive staging; then use the body-free pilot manifests for small measured batches. Never silently refetch or overwrite trusted records. Reaching hundreds still requires sustained source-rich forward collection plus measured selective reacquisition, not a bulk legacy crawl. |
| **Status** | active (forward prevention + safe selective reacquisition path shipped; historical corpus remains thin) |
| **Owner lane** | data / ops |
| **PR/SHA when resolved** | — |
