...
| TD-093 | trusted extraction source fidelity | Combined historic recovery remains three candidates; bounded production reacquisition measured 7/10 identity-supported useful rich sources plus one ambiguous artifact, with two technical failures. Eight artifacts remain pending and 1,219 of 1,227 thin records still lack a staged current-page candidate. | High | active (measured selective path shipped) | `scripts/source_fidelity_recovery.py`; `scripts/reacquire_sources.py`; `docs/v2/BOUNDED-HISTORICAL-REACQUISITION-PILOT-V1.md` |
...
| **Date discovered** | 2026-08-23 |
| **Evidence** | Atomic Extraction Backlog Readiness V1 found 36 ready registry records and 1,227 thin descriptions. Historic recovery found Pink Hudson, Hortifrut, and Lucentlands, while the repeated-body gate correctly blocked 65 Google consent-page conflicts. Bounded Historical Reacquisition Pilot V1 then executed the unchanged production pilot-10: seven identity-supported useful rich artifacts, one rich but ambiguous artifact, one 403, and one timeout. Company newsrooms were 3/3, trade press 2/3, other 2/3 useful plus one ambiguous, and academic 0/1. All eight staged artifacts remain pending. |
| **Impact** | Current readiness remains 36. The prior historic ceiling is 39. This pilot alone could raise it to 43 after seven non-ambiguous affirmations or 44 if the ambiguous artifact is also affirmed; combined unique ceilings are 45/46 because Pink Hudson overlaps the prior set. This is material relative to 36 but still tiny beside 1,227 thin records. Blackberry remained unrecovered and broad historic repair is not demonstrated. |
...
| **Recommended resolution** | Human-review the current eight before a bounded PILOT-25. The observed 70% useful yield supports pursuing the 35 high-priority candidates (cautious 20-30 useful range), especially direct company newsrooms. Keep browser automation out of this lane; record robots/script-rendered outcomes honestly. Reaching hundreds still requires sustained source-rich forward collection, not a bulk legacy crawl. |
