...
| **Evidence** | Same as withdrawn TD-ACQ-001. `PROJECT-STATUS.md` / `app/services/article_dedup.py`: same story under Google-News redirect vs publisher RSS is different URL + `source_id` (recurring draft `ev-media-cec61845f15d790fd055`). Deterministic URL/title+source+date matching cannot merge them without fuzzy title matching (explicitly refused). **More precise root cause found by the Mainstream News + Regulatory Coverage Recall Benchmark V1 mission (2026-08-21):** `MediaOrchestrationService._cross_pipeline_duplicates()` (`app/services/media_orchestration.py:637-666`) filtered its dedup candidate pool to `evidence_role == "publication_artifact"`, excluding legacy trusted Evidence even on exact canonical-URL matches. Collection Relevance + Duplicate Suppression V1 (2026-08-25) closed the remaining operational cost gap: one shared conservative normalizer now removes only allowlisted tracking parameters, preserves semantic queries, includes resolved final URLs, and prefers direct publisher lineage; stable known articles exit before orchestration. A bounded production repeat across Mexico, Morocco, and UK classified 229/229 items unchanged with zero downstream processing, body acquisition, draft creation, retries, or operator actions. |
| **Impact** | Duplicate trusted or pending rows. Operators dismiss by hand. |
| **Workaround** | Inbox cleanup of known duplicates. |
| **Recommended resolution** | Resolved. Collection Runtime + Data Integrity V1 included legacy source-document-shaped trusted Evidence. Collection Relevance + Duplicate Suppression V1 added conservative tracking/redirect identity and early stable-known suppression without fuzzy syndication collapse. |
| **Status** | resolved |
| **Owner lane** | data |
| **PR/SHA when resolved** | — |
| **Regression-test reference** | `tests/test_article_dedup.py`; `tests/test_media_discovery.py`; `tests/test_collection_runner.py`; `tests/test_media_orchestration.py::test_cross_pipeline_dedup_includes_legacy_trusted_publication_without_role`; resolved by PR #70 (`cd107b9`) and PRs #169-#172 (`d91d9f6`) |
...
| **Date discovered** | 2026-09-01 |
| **Evidence** | Industry Pulse Discovery + News Recall V1 live run. Trusted Evidence newest `published_date` is 2026-08-06. The 32-query matrix queries Americas/Europe/Africa/APAC × four berries, but Google News RSS 7d query yield was global 109 vs regional 1/1/1/1, with APAC/Africa qualifying 0. 24h unique hits were 23, of which 22 were correctly non-qualifying consumer/device noise; 1 qualifying unknown-Source item (Pairwise seedless blackberry). `east-fruit.com` Ukraine blueberry variety strategy is SOURCE_KNOWN_NOT_COLLECTED. Retrieval Provider Bake-Off V1 (2026-09-01): Google News `when:` did not change the 18-slice result set; Perplexity Search live (18 calls, $0.09) added 104 URL-unique / 81 host-unique results with 0 URL-identity overlap and 5 shared hosts. Exa/Firecrawl/Bright Data credentials remain absent. Industry Pulse Qualification + Editorial Relevance V1 (2026-09-01): V0 qualifier precision on the frozen 34-row qualification benchmark was 0.720 (7 FPs: cannabis, jobs, livestock, unrelated produce). V1 layered qualifier is 1.000 on that frozen set with 0 recall losses among expected-qualify rows. Live precision still depends on Google News RSS mix (TD-039). |
| **Impact** | A human Google search can still surface a current berry-industry development faster than trusted Evidence. Known-Source collectors (Italian Berry, BlueberryBreeding) can be fresh; the catch-net beyond those hosts is still thin and geographically skewed on the free RSS provider. Qualification is no longer the primary precision bottleneck on the frozen bake-off false-positive classes; residual live FPs are metaphor/ambiguity, not cannabis/jobs/livestock. |
| **Workaround** | Operator `scripts/industry_pulse.py` / POST `/industry-pulse/run`. Optional `scripts/retrieval_bakeoff.py`. Do not auto-onboard pulse hosts. Do not use `captured_date` as publication. Do not silently replace production pulse with a paid provider. |
| **Recommended resolution** | Keep Google News RSS as the production default. Consume the shared `qualify_hit` / `QualificationIndex` for any later Perplexity catch-net activation. Live-test Exa/Firecrawl when keys exist. |
| **Status** | active |
| **Owner lane** | collection / discovery |
| **PR/SHA when resolved** | — |
| **Regression-test reference** | `tests/test_industry_pulse_discovery_v1.py`; `tests/test_industry_pulse_qualification_v1.py` |
