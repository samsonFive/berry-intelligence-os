...
| TD-093 | trusted extraction source fidelity | Deterministic private recovery/re-review finds one exact historic full article (Pink Hudson) and one explicit-lineage Lucentlands transcript; 1,225 of 1,227 thin trusted records still have no historic rich artifact. | High | active (safe recovery path shipped) | `scripts/source_fidelity_recovery.py`; `docs/v2/TRUSTED-SOURCE-FIDELITY-RECOVERY-V1.md` |
...
| **Date discovered** | 2026-08-23 |
| **Evidence** | Atomic Extraction Backlog Readiness V1 found 36 ready registry records, 1,227 thin descriptions, three fictional fixtures, and two duplicates in the 1,268-record production corpus. Trusted Source Fidelity Recovery V1 then indexed all actual local historical artifact stores: Pink Hudson has an exact-ID, exact-URL five-paragraph historic article; one Lucentlands record has an explicit `discovered_item_id` lineage match to a 2,229-segment normalized transcript; zero ambiguities or conflicts were found; 1,225 thin records have no recovered historic artifact. |
| **Impact** | A qualified model could immediately process only a small, almost entirely Blueberry registry subset. After separate human source-fidelity affirmations, one Raspberry article and one Blueberry transcript could join it; Blackberry and broad source-type diversity remain absent. Running the rest of the legacy population would still extract from summaries rather than source material. |
...
| **Recommended resolution** | Human-review the two deterministic pending recoveries, preserve full article bodies on future publication (TD-081 is resolved), acquire/publish real transcripts under the existing gate, and design a separate compare-and-review reacquisition path for the remaining 1,225 records. Never silently refetch or overwrite trusted records. |
| **Status** | active |
| **Owner lane** | data / ops |
| **PR/SHA when resolved** | — |
