...
Review/publish now constructs one `JsonUnitOfWork` over Entity, Fact, Relationship, and Evidence repositories. New Entities and all new Facts/Relationships/Evidence use `create()`. Matched existing Entities use `update()`. The JSON UoW now snapshots each updated record's prior value and restores it in reverse operation order if a later write fails, resolving Part 10.4's documented gap rather than accepting dangling linkage arrays. The inbox draft is deleted only after the UoW exits successfully.
