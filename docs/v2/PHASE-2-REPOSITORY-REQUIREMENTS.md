...
## Part 13 — Phase 2C.2 / BL-034 implementation findings (added 2026-08-14)

`app/repositories/memory/` supplies an independent, schema-validating, defensive-copy backend for all nine families. The same parametrized logical contract runs against JSON and memory, including Source. Representative Reference, Entity Intelligence, Lineage, Scope, Berries Variety, and Intelligence Package exporter paths operate unchanged against memory. Production remains JSON-backed; no Memory Unit of Work was needed because BL-034 proves repository interchangeability, not transaction-engine parity.

Comparison exposed one genuine JSON leak: cached dictionaries returned by `JsonRecordRepository.get()`/`list()` could be mutated without `update()`. Both now return deep copies. No filesystem concepts leak through public contracts, and memory performs no persistence I/O.

BL-035's artifact under `generated/` is disposable because `scripts/build_static.py` replaces that directory. Phase 3's permanent pre-migration archive must live outside `generated/`. Phase 2 implementation is complete and ready for architectural acceptance review; PostgreSQL and Phase 3 remain unstarted.

---

## Part 14 — Phase 2B.3 acceptance-review findings (added 2026-08-14)

**Status:** an independent verification pass over the Phase 2B.3 implementation (commits `aa0f0c6` "Complete Phase 2B.3 transactional write migration" and `486008d` "Harden Phase 2B.3 draft deletion rollback"), checked directly against that task's own brief rather than trusted from its commit messages. Confirmed solid: `JsonUnitOfWork`'s snapshot-and-restore update compensation is correct (verified by tracing the reverse-order restoration chain for repeated updates to the same record — each restore uses the snapshot captured immediately before *that* update, so a chain of N updates correctly unwinds to the true pre-transaction state, not an intermediate one); single-record writes (Signal/Assessment/Recommendation create, Evidence validate/purge, Source mutations) correctly bypass the Unit of Work entirely, per this document's own "not every write needs a Unit of Work" guidance; the failure-injection test suite added by that pass covers all required boundaries (new-Entity create, existing-Entity update, Fact create, Relationship create, Evidence create, draft-deletion-after-success) and asserts exact content equality on rollback, not just record counts.

Five gaps were found against the original task brief and closed in this same pass:

1. **Review/publish's persistence orchestration was still embedded directly in the `review_publish()` route handler**, contradicting the brief's explicit "do not leave a large multi-object transaction embedded directly in the route handler." Extracted into `ReviewPublishService` (`app/services/review_publish.py`): a workflow service that receives repositories and a Unit-of-Work factory (both sourced from `app.composition`, never instantiated ad hoc), performs entity match/create/update orchestration, Fact/Relationship/Evidence creation, and the Draft-success handoff, and returns a plain `PublishResult` — no HTTP, no Jinja, no JSON-path knowledge. The route is now limited to form parsing, the pre-existing required-field/relationship-name validation, and turning the service's result into a response. The service's Draft-lifecycle dependencies (`move_draft_attachments`, `delete_draft`, etc.) are injected by the route rather than imported, since those functions live in `app/main.py` (Drafts have no repository) and importing them into the service module would be circular; the route passes its own current function references at call time, so `monkeypatch.setattr(main, "delete_draft", ...)` in tests is still honored.
2. **`JsonUnitOfWork` was constructed ad hoc inside the route** (`from app.repositories.unit_of_work import JsonUnitOfWork` + direct instantiation), rather than through `app.composition`. Added `get_unit_of_work(data_dir, schemas_dir, *repository_names)` to `app/composition.py` — the one place a caller with genuine multi-object transactional intent builds a `JsonUnitOfWork`, wrapping the same cached repositories `get_repositories()` already returns. Unlike the other `get_*` functions it does not cache its return value (a unit of work is inherently one-shot, scoped to a single logical transaction).
3. **A genuine data-loss bug**, not merely an architectural gap: `move_draft_attachments()` physically moves attachment files out of `inbox/attachments/{draft_id}/` *before* the transactional block begins (the Evidence record's `attachments` field must be populated before the record can be schema-validated and created). If a later step inside the transaction failed and the Unit of Work rolled back the structured records, the attachment files stayed moved — nothing reversed that side effect. A retry's own `move_draft_attachments()` call would then find its source directory already empty and silently produce an Evidence record with `attachments: []`, even though the original draft had real attachments. Fixed with `restore_draft_attachments()` (`app/main.py`, mirrors `move_draft_attachments()` in reverse), called by `ReviewPublishService` if the transaction raises after attachments were moved — a small, targeted compensating action for the one real side effect that needs it, not a generalized new subsystem (attachments still have no repository, and none was created). Proven by `tests/test_app.py::test_publish_attachment_survives_structured_failure_and_retry`, which uploads a real attachment, injects a mid-transaction failure, confirms the file is back in `inbox/` (not stranded at the rolled-back published location), then retries successfully and confirms the published Evidence record's attachment downloads with the correct content.
4. **The Unit-of-Work contract test suite was missing an explicit "successful (non-rollback) update transaction" case** — every existing update-related test exercised the rollback path. Added `test_clean_exit_leaves_an_updated_record_in_its_new_state`, confirming a clean `with` block exit leaves the update committed (no compensation attempted).
5. **Scope discipline**: the original Phase 2B.3 task brief said "do not do BL-034 or BL-035 yet" and "do not proceed into the next phase after completing this report." The implementation proceeded into BL-034 and BL-035 in the two commits immediately following Phase 2B.3, and further into unrelated feature work (a Landscape competitive-theme-matrix UI, a GitHub Pages static-site deploy workflow) without stopping for review at any of those boundaries. Noted here as a process finding, not reversed — the BL-034/BL-035 work was independently reviewed as part of this pass and found sound (see Parts 12-13 above), and undoing already-pushed, already-tested work over a process violation would cost more than it protects.

Validation after all five fixes: 402 tests passed (400 prior + 2 new), `scripts/validate_records.py` zero errors, `scripts/build_static.py` 1,463 pages (unchanged).
